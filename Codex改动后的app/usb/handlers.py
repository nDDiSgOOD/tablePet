...
from ..schemas import InteractionRequest, TtsRequest
...
async def handle_usb_frame(msg_type: int, payload: bytes) -> tuple[int | None, bytes]:
    ...
    if msg_type == USB_CHAT_JSON:
        ...
        result = await process_interaction(
            InteractionRequest(
                device_id=str(data.get("device_id") or device_id),
                user_id=str(data.get("user_id") or data.get("device_id") or device_id),
                source="device_voice",
                transport="usb",
                text=str(data.get("text") or ""),
                want_tts=False,
                extra={"vision": data.get("vision", ""), "history": data.get("history", [])},
            )
        )
        return USB_CHAT_JSON_RESP, usb_json_bytes({"reply": result.assistant_text or ""})
